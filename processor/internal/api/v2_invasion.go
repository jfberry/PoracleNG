...
//
// EXACTLY ONE of {grunt_type, type_id, grunt_id, everything, boss} must be set
// per rule (0 set or >1 set ⇒ 422). All of them resolve to the stored
// grunt_type string the matcher compares against:
...
type v2InvasionRule struct {
	TypeID     *int    `json:"type_id,omitempty" minimum:"1" maximum:"18" doc:"Pokemon type id (1-18); grunt_type resolves to that type's lowercased name. EXACTLY ONE mode field required — set this OR grunt_id OR everything OR boss (not omitted-to-wildcard; use everything for the catch-all). Mutually exclusive with grunt_id/everything/boss. In responses, the active mode field is always present; the other mode fields are omitted/null."`
...
func translateV2Invasion(deps *TrackingDeps, humanID string, profileNo int, oc overrideContext, req *v2InvasionRule) (db.InvasionTrackingAPI, error) {
	...
	if modes > 1 {
		return db.InvasionTrackingAPI{}, huma.Error422UnprocessableEntity(
			"grunt_type, type_id, grunt_id, everything and boss are mutually exclusive (set exactly one)")
	}
