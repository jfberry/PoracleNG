...
func TestQuestMatch_RecordsMatchingDuration(t *testing.T) {
	...
	data := &QuestData{
		PokestopID: "stop1", Latitude: 51.0, Longitude: 0.0,
		Rewards: []QuestRewardData{{Type: 3, Amount: 500}},
	}
	matcher.Match(data, st)

	h, err := metrics.MatchingDuration.GetMetricWithLabelValues("quest")
	if err != nil {
	...
	if got := out.GetHistogram().GetSampleCount(); got != 1 {
		t.Errorf("MatchingDuration{type=quest} sample count = %d, want 1", got)
	}
}

func TestQuestMatch_RecordsCandidateCount(t *testing.T) {
	metrics.MatchingCandidates.Reset()
	human := makeHuman("u1")
	quest1 := &db.QuestTracking{
		ID: "u1", ProfileNo: 1, RewardType: 3, Reward: 0, // stardust any amount
		Distance: 0, Template: "1",
	}
	quest2 := &db.QuestTracking{
		ID: "u2", ProfileNo: 1, RewardType: 3, Reward: 0,
		Distance: 0, Template: "1",
	}
	humans := map[string]*db.Human{
		"u1": human,
	...
	st := makeQuestTestState([]*db.QuestTracking{quest1, quest2}, humans)
	matcher := &QuestMatcher{}
	...
	h, _ := metrics.MatchingCandidates.GetMetricWithLabelValues("quest")
	var out dto.Metric
...
// TestQuestMatch_GeoPrefilterParity asserts flag-on and flag-off produce identical results.
func TestQuestMatch_GeoPrefilterParity(t *testing.T) {
	humans := map[string]*db.Human{
		"u1": {ID: "u1", Enabled: true, Area: []string{"belgium"}, Latitude: 50.5, Longitude: 4.5, CurrentProfileNo: 1},
	}
	rules := []db.QuestTracking{
		{ID: "u1", ProfileNo: 1, RewardType: 3, Reward: 500},
	}
	rulesPointers := make([]*db.QuestTracking, len(rules))
	for i := range rules {
		rulesPointers[i] = &rules[i]
	}
	perHuman := db.PartitionByHuman[db.QuestTracking](rulesPointers)
	...
	for _, flag := range []bool{false, true} {
		st := &state.State{
			Humans:        humans,
			Quests:        rulesPointers,
			QuestsByHuman: perHuman,
			Geofence:      spatial,
			GeoIndex:      state.BuildHumanGeoIndex(humans, nil),
		}
		matcher := &QuestMatcher{GeographicPrefilter: flag}
