...
// TestGymMatch_GeoPrefilterParity asserts flag-on and flag-off produce identical results.
func TestGymMatch_GeoPrefilterParity(t *testing.T) {
	humans := map[string]*db.Human{
		"u1": {ID: "u1", Enabled: true, Area: []string{"belgium"}, Latitude: 50.5, Longitude: 4.5, CurrentProfileNo: 1},
	}
	rules := []db.GymTracking{
		{ID: "u1", ProfileNo: 1, Team: 4, SlotChanges: false, BattleChanges: false},
	}
	rulesPointers := make([]*db.GymTracking, len(rules))
	for i := range rules {
	...
	for _, flag := range []bool{false, true} {
		st := &state.State{
			Humans:      humans,
			Gyms:        rulesPointers,
			GymsByHuman: perHuman,
			Geofence:    spatial,
			GeoIndex:    state.BuildHumanGeoIndex(humans, nil),
		}
		matcher := &GymMatcher{GeographicPrefilter: flag}
