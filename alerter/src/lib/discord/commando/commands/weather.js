exports.run = async (client, msg, command) => {
	...
	try {
		...
		if (client.config.discord.admins.includes(msg.author.id) && msg.channel.isTextBased() && !msg.channel.isDMBased()) target = { id: msg.channel.id, name: msg.channel.name, webhook: false }
		...
		if (!isRegistered && client.config.discord.admins.includes(msg.author.id) && msg.channel.isTextBased() && !msg.channel.isDMBased()) {
		...
		if (!isRegistered && msg.channel.isDMBased()) {
		...
		const resp = await axios.get(`${client.config.processor.url}/api/geocode/forward?q=${encodeURIComponent(command[0].join(' '))}`)
