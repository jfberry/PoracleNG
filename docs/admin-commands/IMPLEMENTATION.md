...
**Scope:** `!untrack <type>` reroute + `!poracle-admin` umbrella with subgroups `slash`, `reload`, `emoji`, `reconcile`, `cache`, `ratelimit`, `summary`, `status`, `maintenance`.
...
### Task 3.3: `!info poracle` convergence

- [ ] Edit `processor/internal/bot/commands/info.go`. The `poracle` subcommand path now calls into the same `statusReport` helper. Verify the existing data the old `!info poracle` displayed is still surfaced (likely a strict superset now).
- [ ] If the old `!info poracle` showed anything `statusReport` doesn't (e.g. a specific field), add it to `statusReport` rather than maintain divergent renders.
- [ ] Update tests for `!info poracle` to assert against the new shared output.
