...
- Poracle routes **all operations** (send, edit, delete) through one per-destination delivery lane, so they arrive strictly in order and a failed request's retries complete before the next operation. Clean-deletes are enqueued onto the same lane via the tracker's clean-delete hook; the one caveat is that under an extreme per-destination backlog a clean-delete is dropped rather than queued (sends/edits block instead) — the receiver's `expires_at` backstop covers exactly that case. There is no ordering guarantee across different destinations. These guarantees make the rules below safe.
