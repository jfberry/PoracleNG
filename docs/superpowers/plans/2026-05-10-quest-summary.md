...
- **Embed layout = description-with-`{{#each}}`** (one bullet per pokestop, name as a clickable map link, AR badge if `withAR`). Discord field-arrays-with-`{{#each}}` won't work in the inline-JSON template path — `{{#each}}` only renders inside string contexts in inline templates, and `@include` doesn't slot fragments into an array either. Splitting the template across `templateFile` + a fragment is functional but inconvenient. **Follow-up (separate PR, not in scope here): a `templateLines: []string` loader convention** (same shape as the existing array-of-strings `description` convention but applied at the template top level) would let inline-JSON templates carry full-on `{{#each}}` over fields without splitting files. Worth doing once the immediate feature is stable.
