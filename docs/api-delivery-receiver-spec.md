...
Poracle serialises **all operations** per destination: every send, edit, and delete for a `destination.id` travels through that destination's own delivery lane, one at a time, in order. A retry of a failed request always completes before the next operation to that destination. **One caveat:** under an extreme per-destination backlog, an expiry-triggered `delete` can be **dropped** rather than queued (sends/edits are never dropped this way — they wait). A dropped delete simply never arrives, which is exactly the case the `expires_at` backstop (§6) covers. There is no ordering guarantee *across different* destinations. These guarantees are what make the rules below safe.
