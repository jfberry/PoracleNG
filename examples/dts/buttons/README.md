...
- **`monster-with-pvp.toml`** — TOML form covering more of the vocabulary: an inline PVP-details response template gated by `show_if = "{{pvpAvailable}}"`, a `pokemon` mute action button, and a single-line `response_text` button. Uses the real monster enrichment fields (`pvpGreat`, `pvpLittle`, `pvpUltra`, each item carrying `rank`/`cp`/`levelWithCap`/`fullName`). Multi-line template strings with `{{#if}}` blocks wrap whole embed fields — the TOML format makes that ergonomic in a way JSON DTS cannot.
