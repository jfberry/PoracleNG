...
- **What does NOT count**:
  ...
  - Jobs marked `BypassRateLimit=true` — used for (a) the rate-limit notification itself and the ban farewell, so the limiter cannot swallow the message reporting on itself, and (b) `questSummary` deliveries from `DispatchQuestSummary`. The summary case is opt-in by the user (the `summary` keyword on a `!quest` rule, after which the user explicitly expects digest messages); silently dropping the digest at delivery time would be more confusing than letting it through. Buffered quests are also naturally bounded by the user's tracked-quest count, so the throughput risk is small. Use `Dispatcher.DispatchBypass(job)` to enqueue these.
