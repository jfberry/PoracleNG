...
- **What does NOT count**:
  ...
  - **Summary bucket**: `questSummary` deliveries use the same `DispatchBypass` channel (to skip the alert bucket) but `DispatchQuestSummary` runs its own `Limiter.CheckSummary(target)` call up-front against the separate `[alert_limits] summary_limit` cap (default 5/window). 1 fire = 1 against the bucket regardless of how many chunks the digest produces. Over-cap dispatches are dropped whole (the buffer is cleared either way) and the first over-cap fire in a window triggers a one-time breach notification via `notifySummaryRateBreach` so the user knows. No ban path — opt-in digests shouldn't escalate to auto-disable.
